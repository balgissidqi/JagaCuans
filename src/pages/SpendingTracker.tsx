...
export default function SpendingTracker() {
          ...
          () =>
            (async () => {
              ...
              try {
                ...
                // Fetch spending with budget info
                const { data: spendingsData, error: spendingsError } =
                  await supabase
                    .from("spending_tracker")
                    .select(
                      `
  ...
  const groupedSpendings = spendings.reduce(
    (groups, spending) => {
      // gunakan date dari user input
      const dateKey = new Date(spending.date).toLocaleDateString("id-ID", {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
  ...
  // urutkan keys dari terbaru ke terlama
const sortedDates = Object.keys(groupedSpendings).sort((a, b) => {
  // konversi kembali ke Date untuk membandingkan
  return new Date(b).getTime() - new Date(a).getTime(); // terbaru dulu
});
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track your expenses </p>
        </div>
  ...
  {Object.keys(groupedSpendings)
    .sort((a, b) => new Date(b).getTime() - new Date(a).getTime()) // terbaru dulu
    .map((date) => (
                ...
                {items.map((spending) => (
                  ...
                  >
                    <div className="flex-1">
                     ...
                     <p className="text-sm text-muted-foreground">
                        {spending.budgeting.category} • {new Date(spending.date).toLocaleDateString('id-ID', {
                          weekday: "long",
                          year: "numeric",
                          month: "long",
                          day: "numeric",
      ...
      {spendings.length === 0 && budgets.length > 0 && (
        <Card className="rounded-2xl shadow-soft p-12 text-center">
          <div className="text-muted-foreground">
            ...
            <p className="text-sm">
              Start tracking your expenses by adding your first spending
            </p>
          </div>
        </Card>
      )}
